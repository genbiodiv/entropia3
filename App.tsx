...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (state.isAutoMode || celebration || showReadme || state.gamePhase === GamePhase.INITIAL) {
      setShowHint(false);
      return;
    }
  ...
  }, [state.round, state.energy, state.isAutoMode, celebration, showReadme, state.gamePhase]);
  ...
  return (
    <div className={`min-h-screen flex flex-col transition-colors duration-500 select-none ${state.isDarkMode ? 'dark bg-slate-950 text-slate-100' : 'bg-amber-50 text-amber-950'}`}>
      <header className={`p-4 md:p-6 border-b flex justify-between items-center sticky top-0 z-50 backdrop-blur-md ${state.isDarkMode ? 'bg-slate-950/80 border-slate-800' : 'bg-white/80 border-amber-100'}`}>
        ...
        <div className="flex items-center gap-2 md:gap-4 justify-end">
          <div className={`px-2.5 py-1.5 rounded-xl border flex items-center gap-2 ${state.isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-amber-200 shadow-sm'}`}>
          ...
          <div className="flex-1 flex flex-col gap-4 md:gap-8">
            <div className="flex-1 flex items-center justify-center relative">
              <div className="w-full h-full max-w-5xl mx-auto flex items-center justify-center">
                <GameBoard particles={state.particles} patterns={state.patterns} gamePhase={state.gamePhase} isDarkMode={state.isDarkMode} />
              </div>
            </div>
            
            <div className={`p-4 md:p-6 rounded-[2rem] md:rounded-[3rem] shadow-xl border flex flex-wrap items-center justify-center gap-3 md:gap-6 ${state.isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-amber-100'}`}>
              <div className="flex items-center gap-3">
                {!isPhaseComplete ? (
                  <div className="flex items-center gap-2 relative">
                    {showHint && (
                      <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-indigo-600 text-white text-[10px] font-bold px-3 py-1.5 rounded-lg shadow-xl animate-bounce whitespace-nowrap z-50">
                        {t.hintText}
                    ...
                    <button 
                      ...
                      className={`h-12 px-5 rounded-xl font-black transition-all flex items-center justify-center gap-2 shadow-md active:scale-95 text-xs whitespace-nowrap ${state.isDarkMode ? 'bg-slate-100 text-slate-900 hover:bg-white' : 'bg-slate-900 text-white hover:bg-slate-800'}`}
                    >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 w-full max-w-3xl">
                  <div className="h-12">
                    {state.gamePhase >= GamePhase.OPEN_SYSTEM && (
                      <button onClick={repairOrder} disabled={state.energy < CONFIG.REPAIR_COST} className="w-full h-full group flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-2 rounded-xl font-black transition-all disabled:opacity-30 disabled:cursor-not-allowed shadow-md text-[10px] sm:text-xs whitespace-nowrap active:scale-95">
                  ...
                  <div className="h-12">
                    {state.gamePhase >= GamePhase.PERSISTENT_PATTERNS && (
                      <button onClick={organizeParticles} disabled={state.energy < CONFIG.ORGANIZE_COST} className="w-full h-full group flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-2 rounded-xl font-black transition-all disabled:opacity-30 disabled:cursor-not-allowed shadow-md text-[10px] sm:text-xs whitespace-nowrap active:scale-95">
                  ...
                  <div className="h-12">
                    {state.gamePhase >= GamePhase.REPRODUCTION && (
                      <button 
                        ...
                        className="w-full h-full group flex items-center justify-center gap-2 bg-amber-500 hover:bg-amber-600 text-white px-2 rounded-xl font-black transition-all disabled:opacity-30 disabled:cursor-not-allowed shadow-md text-[10px] sm:text-xs whitespace-nowrap active:scale-95"
                      >
                  ...
                  <div className="h-12">
                    {state.gamePhase >= GamePhase.REPRODUCTION && (
                      <button 
                        ...
                        className="w-full h-full group flex items-center justify-center gap-2 bg-purple-600 hover:bg-purple-700 text-white px-2 rounded-xl font-black transition-all disabled:opacity-30 disabled:cursor-not-allowed shadow-md text-[10px] sm:text-xs whitespace-nowrap active:scale-95"
                      >
      ...
      {showReadme && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-12">
          <div className={`absolute inset-0 backdrop-blur-sm ${state.isDarkMode ? 'bg-slate-900/60' : 'bg-amber-100/40'}`} onClick={() => setShowReadme(false)}></div>
